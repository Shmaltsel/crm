...
describe("Smoke: School create flow", () => {
  it("сторінка шкіл рендериться з точкою входу створення", async () => {
    server.use(
    ...
    await waitFor(() => {
      expect(screen.getByLabelText("Додати школу")).toBeInTheDocument();
    });
  });
});
