...
export default function EventReport() {
  ...
  if (!event) return <div className="p-8 text-slate-500">Подію не знайдено</div>;
  ...
  return (
    <div className="p-8 bg-slate-50 min-h-screen">
      ...
      <button onClick={() => history.back()} className="mb-4 px-4 py-2 bg-white border border-slate-200 rounded-lg text-sm text-slate-600 hover:bg-slate-50 flex items-center gap-2">
        ← Назад
      </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
          ...
          <div className="space-y-2 text-sm">
            ...
            <Row label="Адреса" value={event.address} />
            <Row label="Дата" value={new Date(event.date).toLocaleDateString('uk-UA')} />
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
          ...
          <div className="space-y-2 text-sm">
            ...
            <Row label="Вчителі задоволені" value={report?.teachersSatisfied ? 'Так' : 'Ні'} />
