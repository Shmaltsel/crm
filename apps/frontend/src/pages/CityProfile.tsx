...
import CityAnalytics from "../components/city-profile/CityAnalytics";
...
export default function CityProfile() {
  ...
  const [crewForm, setCrewForm] = useState({ name: "", hostId: "", driverId: "" });

  const fetchCityAndUsers = async () => {
    try {
      const [cityRes, usersRes] = await Promise.all([
        api.get(`/cities/${id}`),
        api.get("/users")
      ]);
  ...
  const handleCreateCrew = async (e: React.FormEvent) => {
    ...
    try {
      await api.post(`/cities/${city?.id}/crews`, crewForm);
      setIsCreateCrewModalOpen(false);
      fetchCityAndUsers(); // Оновлюємо дані
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleDeleteCrew = async (crewId: string) => {
    ...
    try {
      await api.delete(`/cities/crews/${crewId}`);
      fetchCityAndUsers();
    } catch (err) {
      console.error(err);
    }
  ...
  const availableHosts = users.filter(u => u.role === "HOST" && u.city?.id === city.id && !busyUserIds.includes(u.id));
  const availableDrivers = users.filter(u => u.role === "DRIVER" && u.city?.id === city.id && !busyUserIds.includes(u.id));

  const totalChildren = completedEvents.reduce((sum, ev) => sum + (ev.report?.childrenCount || ev.childrenPlanned || 0), 0);
  const totalRevenue = completedEvents.reduce((sum, ev) => sum + (ev.report?.totalSum || ev.price || 0), 0);
  const totalProfit = completedEvents.reduce((sum, ev) => sum + (ev.report?.remainderSum || 0), 0);
        ...
        {TABS.map((tab) => (
          <button
            ...
            className={`flex items-center justify-center sm:justify-start gap-1.5 sm:gap-2 px-2 sm:px-5 py-2.5 rounded-lg text-xs sm:text-sm font-medium transition-colors ${
              activeTab === tab.key ? "bg-blue-600 text-white shadow-sm" : "text-slate-500 hover:text-slate-700 hover:bg-slate-50"
            }`}
          >
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col">
            <div className="p-5 sm:p-6 border-b border-slate-100 flex justify-between bg-slate-50">
              ...
              <button onClick={() => setIsCreateCrewModalOpen(false)} className="text-slate-400 hover:text-slate-600 text-lg leading-none">✕</button>
            </div>
            <form onSubmit={handleCreateCrew} className="p-5 sm:p-6 space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-emerald-600 mt-1">✓ Доступно: {availableHosts.length} вільних</p>
              </div>
              <div>
