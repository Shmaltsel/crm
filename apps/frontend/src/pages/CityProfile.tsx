...
export default function CityProfile() {
  ...
  const totalChildren = completedEvents.reduce(
    (sum, ev) => sum + (ev.report?.childrenCount || ev.childrenPlanned || 0), 0
  );
  const totalRevenue = completedEvents.reduce(
    (sum, ev) => sum + (ev.report?.totalSum || ev.price || 0), 0
  );
  const totalProfit = completedEvents.reduce(
    (sum, ev) => sum + (ev.report?.remainderSum || 0), 0
  );
                ...
                <tbody>
                  {crews.map((crew) => (
                    <tr key={crew.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                      <td className="p-4 font-medium text-slate-800">{crew.name}</td>
                      <td className="p-4 text-blue-600 font-medium">{crew.host?.name || "—"}</td>
                      <td className="p-4 text-emerald-600 font-medium">{crew.driver?.name || "—"}</td>
                      <td className="p-4 text-slate-600">{crew.car || "—"}</td>
                      <td className="p-4 text-slate-600"><PhoneLink phone={crew.phone} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
