...
export default function CityProfile() {
  ...
  const handleCreateCrew = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCrewForm({ name: "Екіпаж 1", hostId: "", driverId: "" });
      fetchCity(); // Оновлюємо місто
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const busyUserIds =
    city?.crews?.flatMap((c: any) => [c.hostId, c.driverId]) || [];
  const availableHosts = users.filter(
    (u) => u.role === "HOST" && !busyUserIds.includes(u.id),
  );
  const availableDrivers = users.filter(
    (u) => u.role === "DRIVER" && !busyUserIds.includes(u.id),
  );
            ...
            {crews.map((crew: any) => (
              <tr key={crew.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                ...
                <td className="p-4 text-center">
                  <button onClick={() => handleDeleteCrew(crew.id)} className="text-slate-400 hover:text-red-500">🗑</button>
                </td>
              </tr>
            ))}
          </tbody>
