...
export default function SchoolProfile() {
  ...
  const [schoolData, setSchoolData] = useState<any>({
    id: "", cityId: "", name: "", type: "Школа", city: "", address: "", director: "", phone: "", email: "", childrenCount: 0, notes: "",
  });
  ...
  const [commentModal, setCommentModal] = useState({ isOpen: false, mode: "pipeline", stepId: null as number | null, historyId: null as string | null, text: "" });

  const [editForm, setEditForm] = useState(schoolData);
  const [eventForm, setEventForm] = useState({ project: "Голограма для школи", date: "", time: "11:00", childrenPlanned: "", price: "", address: "", contactPerson: "", contactPhone: "" });
  ...
  const fetchData = async () => {
    try {
      ...
      if (schoolRes.data) {
        setSchoolData({
          id: schoolRes.data.id, cityId: schoolRes.data.cityId, name: schoolRes.data.name || "", type: schoolRes.data.type || "Школа", city: schoolRes.data.city?.name || "", address: schoolRes.data.address || "", director: schoolRes.data.director || "", phone: schoolRes.data.phone || "", email: schoolRes.data.email || "", childrenCount: schoolRes.data.childrenCount || 0, notes: schoolRes.data.notes || "",
        });
  ...
  const currentStageIndex = PIPELINE_STAGES.findIndex((s) => s.key === currentEvent?.status) !== -1 ? PIPELINE_STAGES.findIndex((s) => s.key === currentEvent?.status) : 0;
  const creatorName = currentEvent?.history?.length > 0 ? currentEvent.history[currentEvent.history.length - 1].userName : "Немає даних";
  ...
  const handlePipelineClick = (stepId: number) => {
    ...
    setCommentModal({ isOpen: true, mode: "pipeline", stepId: nextStage.id, historyId: null, text: "" });
  };
  ...
  const handleSaveComment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (commentModal.mode === "pipeline") {
        ...
        const res = await api.patch(`/events/${currentEvent.id}/status`, { status: nextStage.key, actionName: `Етап пройдено: ${activeStage.name}`, comment: commentModal.text }, { headers });
        if (nextStage.key === "RE_SALE") {
          setExitingEventId(currentEvent.id);
          setTimeout(() => {
            setEvents((prev) => prev.filter((ev) => ev.id !== currentEvent.id));
            setSelectedEventId(null); setExitingEventId(null);
          }, 500);
        } else {
          setEvents((prev) => prev.map((ev) => ev.id === currentEvent.id ? { ...ev, ...res.data } : ev));
        }
      } else if (commentModal.mode === "history" && commentModal.historyId) {
        await api.patch(`/events/history/${commentModal.historyId}`, { comment: commentModal.text }, { headers });
  ...
  const handleSaveEvent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await api.post("/events", payload, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
  ...
  const handleSaveSchoolInfo = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await api.patch(`/schools/${id}`, editForm, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
  ...
  const handleUpdatePreparation = async (field: string, status: string) => {
    if (!currentEvent) return;
    try {
      await api.patch(`/events/${currentEvent.id}/preparation`, { field, status }, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
      setEvents((prev) => prev.map((ev) => ev.id === currentEvent.id ? { ...ev, preparation: { ...(ev.preparation || {}), [field]: status } } : ev));
    } catch (e) { console.error("Помилка оновлення підготовки", e); }
  };
  ...
  const handleSubmitReport = async (reportData: any) => {
    ...
    try {
      ...
      await api.patch(`/events/${currentEvent.id}/status`, { status: "RE_SALE", actionName: "Звіт сформовано. Захід завершено." }, { headers });
      setExitingEventId(currentEvent.id);
      setTimeout(() => {
        setEvents((prev) => prev.filter((ev) => ev.id !== currentEvent.id));
        setSelectedEventId(null); setExitingEventId(null);
  ...
  const handleAssignCrew = async (crewId: string) => {
    try {
      const res = await api.post(`/events/${currentEvent.id}/assign-crew`, { crewId }, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
  ...
  return (
    <div className="p-4 md:p-8 bg-slate-50 min-h-screen text-slate-800 font-sans w-full overflow-x-hidden pb-24 md:pb-8">
      ...
      <SchoolProfileHeader 
        ...
        onEdit={() => { setEditForm(schoolData); setIsEditModalOpen(true); }}
      ...
      <div className="flex flex-col xl:flex-row gap-6">
        ...
        <div className={`flex-1 flex flex-col gap-6 transition-all duration-500 ease-in-out transform origin-top ${exitingEventId === currentEvent?.id ? "opacity-0 scale-95 -translate-y-4 pointer-events-none" : "opacity-100 scale-100 translate-y-0"}`}>
          {currentEvent && <Pipeline currentStageIndex={currentStageIndex} currentEvent={currentEvent} onPipelineClick={handlePipelineClick} stages={PIPELINE_STAGES} />}

          {currentEvent && currentStageIndex >= 4 && (
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              <EventPreparation data={currentEvent.preparation || {}} onUpdate={handleUpdatePreparation} onOpenCrewModal={() => setIsCrewModalOpen(true)} />
              <AssignedCrew currentEvent={currentEvent} employees={users} />
            </div>
          )}

          <EventDetails currentEvent={currentEvent} schoolName={schoolData.name} cityId={schoolData.cityId} onEventUpdated={fetchData} />
          <EventsTable events={events} selectedEventId={selectedEventId} onEventSelect={setSelectedEventId} onDeleteSuccess={fetchData} />
        </div>
      </div>
      ...
      <ReportModal isOpen={isReportModalOpen} onClose={() => setIsReportModalOpen(false)} onSave={handleSubmitReport} schoolName={schoolData.name} eventType={currentEvent?.project} eventDate={currentEvent?.date} eventIndex={events.filter((e) => e.schoolId === schoolData.id).indexOf(currentEvent!) + 1} />
    </div>
  );
}