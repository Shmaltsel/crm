...
export default function FunnelBar({ funnel }: Props) {
  ...
  return (
    <div>
      ...
      {/* Мобільна версія: flex + overflow-x-auto (горизонтальний скрол)
        Десктоп версія: grid (сітка) 
      */}
      <div className="flex flex-row md:grid md:grid-cols-6 gap-2 md:gap-3 overflow-x-auto pb-2 scrollbar-hide">
        {visibleStages.map((stage) => {
          ...
          return (
            <button
              ...
              className={`
                flex flex-col items-center md:items-start p-2.5 md:p-3.5 rounded-xl border text-center md:text-left
                transition-all duration-150 bg-white min-w-[80px] md:min-w-0
                ${stage.border} hover:shadow-sm hover:border-opacity-80 active:scale-[0.98]
              `}
            >
              {/* Іконка прихована на мобільних, видно тільки на ПК */}
              <div className={`hidden md:flex w-8 h-8 rounded-lg ${stage.bg} items-center justify-center text-base mb-2.5`}>
                {stage.icon}
              </div>
              
              <span className={`text-xl md:text-2xl font-bold leading-none ${stage.color}`}>
                {count}
              </span>
              
              <span className="text-[10px] md:text-xs font-semibold text-slate-700 mt-1.5 leading-tight truncate w-full">
                {stage.name}
              </span>
              
              {/* Підзаголовок приховано на мобільних для економії простору */}
