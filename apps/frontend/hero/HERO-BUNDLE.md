...
*(100×100px SVG rocket with gradient body, window, fins, 3-layer flame animation, 40-particle trail — see source in repo)*

## overlays/PortalOverlay.tsx

*(3 SVG planets: teal/Голограма, coral/Малювайка, pink/Popify — radial gradients, rings, glow, drift animation)*

## overlays/NebulaOverlay.tsx

*(Scroll-driven gradient background with purple/coral depth layers)*

## overlays/StarField.tsx

*(70 twinkling stars, 3 size tiers, 4 colors: gold/teal/coral/cream)*

## overlays/GrassGround.tsx

*(Interactive grass blades + tree, mouse-reactive skew)*
...
*(Web Audio API: wind noise lowpass + random chimes every 5.5–13s)*

## hooks/useBeatStrength.ts

*(Single beat strength from scroll progress, 13 beats, 28% fade zones, smoothstep)*

## hooks/useBeatStrengths.ts

*(Array of 13 MotionValues via useTransform)*
...
*(Waypoint interpolation for rocket trajectory)*

## hooks/useScrollSnap.ts

*(Snap to nearest beat center after 180ms scroll stop, skip at extremes)*

## hooks/useScrollStory.ts

*(useScroll from framer-motion on container ref)*
