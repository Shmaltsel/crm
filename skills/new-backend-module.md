...
8. **Верифікація критеріїв**
   - Прочитати `.agents/tasks/<id>.md` → секцію `criteria`.
   - Перевірити кожен пункт критеріїв. Якщо якийсь не виконано — НЕ позначати done.

9. **Checkpoint** (якщо є `checkpoint_at`)
   - Зупинись на проміжному стані, вказаному в `checkpoint_at`.
   - Виклич `notify()` з коротким описом прогресу перед продовженням.

10. **Верифікація перед комітом**
